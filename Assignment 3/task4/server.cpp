...
int main(int argc, char *argv[])
{
	...
	// cout<<sizeof buffer<<endl;
		...
		while(exiting){
			...
			if (!strncasecmp(buffer,listserver,strlen(listserver)))
			{
			...
			else if (!strncasecmp(buffer,deleteserver,strlen(deleteserver))){
				clientfiles=files();
				istringstream string_extraction(buffer);
				string string_distribution[3];
				...
				getline(string_extraction, string_distribution[1], ' ');
				getline(string_extraction, string_distribution[2]);
				cout<<"Client: Delete server "<<string_distribution[2]<<endl;
				string ch;
				string filename=string_distribution[2];
				if(find(clientfiles.begin(), clientfiles.end(), filename) != clientfiles.end()) {
...
//////////////////////////////////////////////////////////////////
					...
					int data=filesize;
					while((data>0)){
						length=recv(server,buffer,buflen,0);
				...
				if(find(clientfiles.begin(), clientfiles.end(), filename) == clientfiles.end()) {
					...
					cout<<"Client: Send "<<filename<<endl;
				// 	struct stat file_s;
				...
				// 	int senddata=0;
				// 	while (((senddata = sendfile(server,file,&checkset,filesize)) > 0) && (filesize>0)){
				// 		cout<<filesize<<endl;
				// 		filesize-=senddata;
				// 	memset(buffer,0,buflen);
				// 		cout<<filesize<<endl;
				...
				// 		}
					...
					int senddata=0;
					while (((senddata = sendfile(server,file,&checkset,buflen)) > 0) && (filesize>0)){
						filesize-=senddata;
					//memset(buffer,0,buflen);
